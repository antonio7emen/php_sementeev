<html>
<head> <title> Добавление новой записи </title> </head>
<body>
<H2>Добавление новой записи:</H2>
<form action="save_new_os.php" metod="get">
<br>Название: <input name="name" size="20" type="text">
<br>Оборудование: <input name="equipment" size="20" type="text">
<br>Разрядность: <input name="bitness" size="10" type="text">
<br>Разработчик: <input name="developer" size="20" type="text">
<br>Пользователей, млн.: <input name="population" size="10" type="text">
<p><input name="add" type="submit" value="Добавить">
<input name="b2" type="reset" value="Очистить"></p>
</form>
<p>
<a href="index.php"> Вернуться </a>
</body>
</html>